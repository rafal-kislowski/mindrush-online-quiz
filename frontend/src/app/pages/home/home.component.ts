...
export class HomeComponent {
  ...
  readonly joinCodeIndices = Array.from({ length: this.joinCodeLength }, (_, i) => i);
  ...
  readonly leaderboard: Array<{ name: string; points: number; badge: 'gold' | 'silver' | 'bronze' | 'none' }> = [
  ...
  onJoinCodeChange(value: string): void {
    ...
    this.joinCodeChars = Array.from({ length: this.joinCodeLength }, (_, i) => this.joinCode[i] ?? '');
  }
}
